...
// Generate event list
fetch("http://localhost:3000/api/events").then((resp)=>resp.json()
).then((data)=>{
    for(let i = 0; i < data.length; i++){
        ...
        // event date
        for(let date1 = 0; date1 < data[i].dates.length; date1++){
            const thDate = document.createElement("th");
            thDate.innerText = data[i].dates[date1].date;
        ...
        for(let index1 = 0; index1 < data[i].dates.length; index1++){
            let numberOfV = 0;
            const tdNumberOfV = document.createElement("td");
            trNumbOfPart.appendChild(tdNumberOfV);
            for(let idk = 0; idk < data[i].dates[index1].attendees.length; idk++)if (data[i].dates[index1].attendees[idk].available === true) numberOfV++;
        ...
        addMember.addEventListener("click", (e1)=>{
            ...
            pushButton.addEventListener("click", (e)=>{
                ...
                const date = [];
                for(let index = 1; index < numberOfChild; index++)date.push({
            ...
            newTd.appendChild(name1);
            newTr.appendChild(newTd);
            // edit available
            for(let index3 = 0; index3 < data[i].dates.length; index3++){
